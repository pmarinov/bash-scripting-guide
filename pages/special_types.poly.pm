...
The ◊code{shift} command reassigns the positional parameters, in
effect shifting them to the left one notch.

◊code{$1 <--- $2, $2 <--- $3, $3 <--- $4, etc.}

The old ◊code{$1} disappears, but ◊code{$0} (the script name) does not
change. If you use a large number of positional parameters to a
script, shift lets you access those past 10, although
$◊escaped{◊"{"}bracket$◊escaped{◊"}"} notation also permits this.

◊strong{Using ◊code{shift}}
...
The ◊code{shift} command can take a numerical parameter indicating how
...
passed to a function. TODO: See Example 36-18.
