...
◊definition-entry[#:name "ssh"]{
Secure shell, logs onto a remote host and executes commands
there. This secure replacement for ◊command{telnet},
◊command{rlogin}, ◊command{rcp}, and ◊command{rsh} uses identity
...
Caution: Within a loop, ◊command{ssh} may cause unexpected
behavior. According to a Usenet post in the comp.unix shell archives,
◊command{ssh} inherits the loop's stdin. To remedy this, pass
◊command{ssh} either the ◊code{-n} or ◊code{-f} option.

}

◊definition-entry[#:name "scp"]{
Secure copy, similar in function to ◊command{rcp}, copies files
between two different networked machines, but does so using
authentication, and with a security level similar to ◊command{ssh}.
...
user. The ◊command{mesg} command may, of course, be used to disable write access
to a terminal
